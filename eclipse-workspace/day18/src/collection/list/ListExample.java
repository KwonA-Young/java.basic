...
public class ListExample {
	public static void main(String[] args) {
		// ArrayList와 LinkedList는 사용양식은 전부 List를 따른다.
		// 따라서 교환이 가능하다. 인터페이스가 같은 구현체들은 사용법이 같다.
		// 제네릭: 타입 안정화를 위해 객체 내부에 저장되는 타입을 알치시키는 방법
		// String만 저장할 수 있는 컬렉션 
		List<String>  list = new ArrayList<>(); // 1.8부터는 생성자에 제네릭 표기 안해도 됨 
		// List<String>  list = new LinkedList<>();
		
		String str1 = "Java";
		String str2 = "SpringBoot";
		System.out.println(list); // 컬렉션 특성상 toString()이 이미 재정의되어 주소가 안나오고 내부요소가 나온다.
		...
		// list에 저장된 총 객체수 얻기 size()
		int size = list.size();
		System.out.println("list에 저장된 총 객체의 수 : " + size);
		
		// list에 객체 삽입 add(index, 객체) : 데이터를 해더 인덱스에 삽입
		list.add(2, "Network");
		System.out.println(list);
		
		// list 데이터 수정 set(index, 객체)
		// list[3] = "PostgreSQL"; // 일반 수정 구문 사용불가
